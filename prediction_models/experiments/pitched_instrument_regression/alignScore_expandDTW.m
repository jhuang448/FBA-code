function [midi_mat_aligned, notes_altered, dtw_cost, path, jump] = alignScore_expandDTW( midi_path, f0_wav, wav, fs_w, wSize, hop )
...
[extend_midi, map] = extendMidi(midi_mat);% add length info to midi contour; map for getting back
D = wrappedDist(extend_midi, wav_pitch_contour_in_midi, 12);
%[path, C] = ToolSimpleDtw(D);
[path, C, jump] = RevisedDtw_expand(D', idx, map);
C = C';
path = [path(:, 2), path(:, 1)];
...
% map the extended midi back to original midi
dx = diff(ix);
pos = find(dx);
path = path([1 pos+1], :);
idx_mp = [];
idx_path = [];
for i=1:size(path, 1)
    if ~isempty(find(path(i, 1)==map))
        idx_mp = [idx_mp i];
        idx_path = [idx_path find(map==path(i,1))];
    end
end
path = path(idx_mp, :);
path(:, 1) = idx_path;
...
midi_mat = midi_mat(path(:,1), :); %update midi_mat, add new notes (due to jumps)
midi_mat_aligned = midi_mat;
midi_mat_aligned(1:end,6) = time_pos; % replace time in midi with time in recording
